...
import { Package, FlaskConical, Search } from 'lucide-react';

interface IngredientData {
  name?: string; type?: string; unit?: string; price?: number;
  stock?: number; format?: number; homemade?: boolean;
  brand?: string; family?: string; yield?: number;
  yieldUnit?: string; preparationType?: string;
}

interface TeamIngredientRow {
  id: string; team_id: string; created_by: string;
  data: IngredientData; updated_at: string;
}

interface Props {
  ingredients: TeamIngredientRow[];
  teamId: string; userId: string; isManager: boolean;
...
const STOCK_DOT: Record<string, string> = {
  full: 'bg-emerald-400', low: 'bg-orange-400', empty: 'bg-red-400',
};

export default function TeamStockClient({ ingredients, teamId, userId, isManager, teams }: Props) {
  ...
  return (
    <main className="px-4 py-4 pb-safe space-y-4">
      {teams.length > 1 && (
        <select
          defaultValue={teamId}
          onChange={(e) => router.push(`/communication/bar?team=${e.target.value}`)}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-[var(--text-dim)] mt-1">Ajoutez les premiers ingrédients du bar</p>
          </div>
          <Link href={`/communication/bar/new?team=${teamId}`} className="btn-primary px-4 py-2 text-sm">
            <Package size={14} /> Ajouter
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {filtered.map((ing) => {
            ...
            return (
              <Link key={ing.id} href={`/communication/bar/${ing.id}?team=${teamId}`}
                className="card block hover:border-[var(--gold-dim)] transition-colors">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    {isHomemade && <FlaskConical size={14} className="text-blue-400 shrink-0" />}
